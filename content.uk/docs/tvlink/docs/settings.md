...
#### step3_cv_high (0.25)
Що: Верхній поріг «сильно скаче» (коефіцієнт варіації).  
Більше: Рідше переходимо до захисного режиму при нестабільності.  
Менше: Частіше діагностуємо нестабільність і переходимо до захисного режиму.  
Рекомендація: 0.15–0.35.  
Коли: Рідко.

---

### 3.3. Drift — наздоганяння та відставання

#### drift_max_s (1.5)  [тільки smart]
Що: Максимальне відставання (секунди), при якому починаємо «наздоганяти».  
Більше: Рідше наздоганяємо.  
Менше: Частіше наздоганяємо, менше буферизації, але більше запитів.  
Рекомендація: 1.0–3.0.  
Коли: Якщо потік дуже повільний або навпаки занадто швидкий.

#### drift_accel_ratio (0.5)  [тільки smart]
Що: Наскільки сильно прискорюємо перевірки при відставанні (частка від звичайного очікування).  
Більше: Менше прискорення.  
Менше: Сильне прискорення, швидше наздоганяємо.  
Рекомендація: 0.3–0.8.  

---

### 3.4. Batch і VOD

#### batch_factor (1.5)  [тільки smart]
Що: При скільки нових сегментах одночасно підлаштовуємо швидкість перевірок.  
Більше: Рідше реагуємо на «пакети» сегментів.  
Менше: Частіше підлаштовуємося.  
Рекомендація: 1.2–2.0.

#### vod_accel_segments (3)  [тільки smart]
Що: Скільки нових сегментів після розриву вважаємо «швидкою фазою» відновлення.  
Більше: Довша швидка фаза.  
Менше: Коротша швидка фаза.  
Рекомендація: 2–5.

---

### 3.5. Replay — повтор останнього

#### replay_segments (3)  [майже тільки smart]
Що: Скільки останніх сегментів можна «прокручувати» при Gap.  
Більше: Довший повтор, картинка довше не «завмерла».  
Менше: Коротший повтор.  
0: Вимкнути повтор.  
Рекомендація: 2–5.  
Коли: Якщо не подобається повтор або навпаки треба довший.
...
#### gap_threshold_s (30)
Що: Через скільки секунд «тиші» зупинити потік.  
Більше: Довше чекаємо перед зупинкою.  
Менше: Швидше зупиняємося.  
Рекомендація: 15–60.  
Коли: Завжди. Найчастіше змінюваний параметр.

#### gap_countdown_s (10)  [тільки smart]
Що: За скільки секунд до зупинки починати зворотний відлік і повтор останніх сегментів.  
Більше: Раніше починаємо повтор.  
Менше: Пізніше починаємо повтор.  
Рекомендація: 5–15.
...
**Не впевнені**, які параметри змінювати?

**Простий підхід**: змінюйте лише `gap_threshold_s` (через скільки секунд зупинятися) і `replay_segments` (скільки останніх шматків повторювати).

**Середній підхід**: додатково підлаштуйте `step3_u1_ratio` і `step3_u2_ratio` (як швидко реагувати на тишу), `drift_max_s` (коли починати наздоганяти).

**Експертний підхід**: змінюйте все, розуміючи що робите.

**За замовчуванням**: майже завжди працює добре.

Кінець розділу Smart.
