...
+ «Streamlink session pool» - це розмір фіксованого пулу із заздалегідь ініціалізованих сесій Streamlink.
Фіксований розмір пулу гарантує, що одні й ті самі буфери будуть повторно використовуватись.
Пам'ять під «Stream Ring buffer» та черги буде виділена один раз на кожну сесію пулу і надалі лише повторно використовуватиметься.
Результат: обсяг оперативної пам'яті (RAM) підніметься до _N * (розмір Ring Buffer) + (накладні витрати)_ і перестане зростати.
Рекомендації:
  - Встановіть значення, що дорівнює максимальному числу одночасних потоків + кількість користувачів.
  - Для одного-двох клієнтів залиште значення «4» (за замовчуванням).
...
+ «Use FFmpeg muxer» – цей параметр вмикає/вимикає використання внутрішнього FFmpeg мультиплексора (muxer) Streamlink.
Якщо параметр увімкнено (за замовчуванням) і в системі встановлено «FFmpeg», до вихідного потоку будуть додані всі аудіодоріжки, наявні в HLS-плейлисті.
...
2. програма використовує більше ніж 100 МБ оперативної пам’яті для систем, де обсяг ОЗП менший за 1,5 ГБ, та більше ніж 200 МБ для систем, де обсяг ОЗП перевищує 1,5 ГБ;
...
Для режимів «Live Smart/Local VOD» діє <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/GAP_THRESHOLD_UA.md">інша логіка зупинки потоків</a>.
...
Частково відповідає параметру <a target='_blank' href="https://streamlink.github.io/cli.html#cmdoption-hls-playlist-reload-time">«hls-playlist-reload-time»</a> у «Streamlink».
    ...
    Це заощаджує ресурси та прискорює потік.
    Загальний опис цього режиму наведено <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/README_SMART_UA.md">тут</a>.
    Детальний опис цього режиму наведено <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/SMART-BUFFER-MODE-UA.md">тут</a>.
...
+ «HLS Live buffer multiplier» – цей параметр визначає коефіцієнт множення для цільового буфера потокового відео у режимі реального часу (Live).
Працює тільки тоді, коли для параметра «HLS playlist reload time» встановлено значення «smart».
Детальніше <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/SMART-BUFFER-MODE-UA.md">тут</a>.

+ «HLS VOD buffer multiplier» – цей параметр визначає коефіцієнт множення для цільового буфера VOD-потоків (наприклад, IPTV-архівів).
Детальніше <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/SMART-BUFFER-MODE-UA.md">тут</a>.

+ «VOD Limit segments on startup» – цей параметр визначає кількість сегментів, які будуть завантажені та додані до черги на початку VOD-потоку.
Його можна розглядати як буфер, подібний до параметра «HLS live edge» для потокового відео у режимі реального часу (Live).
Детальніше <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/README_SMART_UA.md">тут</a>.

+ «VOD Limit segments in progress» – цей параметр визначає кількість сегментів, які будуть додані до черги у процесі перезавантаження списку відтворення.
Детальніше <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/README_SMART_UA.md">тут</a>.

+ «VOD segments queue Step» – кількість сегментів за один крок після ривка (burst). Забезпечує швидкий старт та рівномірне завантаження.
Детальніше <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/README_SMART_UA.md">тут</a>.

+ «Close segment connection» – цей параметр додає заголовок "Connection: close" до HTTP-запитів для сегментів, ключів і списків відтворення (плейлистів).
Це корисно при виникненні проблем із механізмом keep-alive (наприклад, зависання).
Детальніше <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/README_SMART_UA.md">тут</a>.
...
Детальна інформація по деяких аспектах роботи програми:
+ <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/README_SMART_UA.md">README_SMART</a>
+ <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/SMART-BUFFER-MODE-UA.md">SMART-BUFFER-MODE</a>
+ <a target='_blank' href="https://github.com/AlexELEC/TVLINK-Releases/blob/main/docs/GAP_THRESHOLD_UA.md">GAP_THRESHOLD</a>
...
+ Allow IPs - дозволяє використовувати це джерело тільки із зазначених IP-адрес (розділених пробілом). Пусте поле означає, що дозволено всім.
