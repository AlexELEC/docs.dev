...
+ «Check internet (host:port)» – це адреса (хост і порт), за якою «TVLINK» перевіряє підключення до інтернету.
Хост може бути як доменним іменем, так і IP-адресою, наприклад «8.8.8.8:53». За замовчуванням використовується «google.com:443».
Якщо вам потрібно змінити цю опцію перед запуском програми, створіть файл «check-internet.ip» у каталозі tvlink та впишіть туди потрібну адресу.
...
Частково відповідає параметру <a target='_blank' href="https://streamlink.github.io/cli.html#cmdoption-hls-playlist-reload-time">«hls-playlist-reload-time»</a> у «Streamlink».
    + «segment» - має те саме значення, що й у «Streamlink». Для сегмента використовується час, указаний тегом «#EXTINF».
    + «targetduration» - відповідає «default» у «Streamlink». Для всього списку сегментів використовується час, указаний тегом «#EXT-X-TARGETDURATION».
    + «smart» — це режим, у якому програма самостійно вирішує, коли запитувати оновлення, щоб отримувати сегменти без зайвого очікування та не робити надто частих запитів.
    Це заощаджує ресурси та прискорює потік. Детальний опис цього режиму наведено [нижче](/docs/tvlink/docs/settings/#режим-smart-для-оновлення-hls-плейлиста).
...
+ «VOD Limit segments on startup» – цей параметр визначає кількість сегментів, які будуть завантажені та додані до черги на початку потоку.
Його можна розглядати як буфер, подібний до параметра «HLS live edge» для потокового відео у режимі реального часу (Live).

+ «VOD segments queue Step» – цей параметр вказує, скільки сегментів одночасно додавати в чергу для завантаження після того, як спрацював параметр «VOD Limit segments on startup».
...
## Пояснення щодо опцій «VOD Limit segments on startup» та «VOD segments queue Step»

TVLINK може працювати з Catchup HLS-потоками майже так само, як і зі звичайними Live HLS-потоками.

Як ви знаєте, основне завдання TVLINK — це робота з Live HLS-потоками. Для цього програма має якомога швидше завантажувати сегменти потоку в кілька черг.
Такий підхід не підходить для Catchup HLS-потоків (IPTV-архівів), які, по суті, є не Live, а VOD (Video on Demand) потоками. Причина в тому, що сегменти завантажуються занадто швидко.

Пояснімо на прикладі. Ви ввімкнули потік, що підтримує IPTV Catchup, і перемотали відео на 1 хвилину назад.
Ваш плеєр (наприклад, Kodi) надсилає запит серверу TVLINK приблизно такого вигляду:
...
+ {current_time - 60} — відповідно, поточний час мінус 60 секунд

TVLINK обробляє запит і пересилає його вашому IPTV-провайдеру. У відповідь сервер провайдера повертає плейлист, який містить усе відео за цю хвилину.
Наприклад, це 10 сегментів по 6 секунд. TVLINK завантажує їх за кілька секунд і бачить кінець відео (EOF).
Це призводить до перезапуску потоку з новими параметрами utc/lutc, але знову на одну хвилину. У цей час Kodi постійно отримуватиме обривки відео і «заїкатися». І так по колу.

Щоб запобігти цьому, TVLINK імітує роботу Live-потоків для Catchup VOD-потоків.

Логіка, яка працювала раніше для VOD:

+ Запит плейлиста.
+ Завантаження всіх сегментів, які є в плейлисті, якомога швидше.
+ Скидання та новий запит плейлиста.

Логіка, яка працює зараз для VOD:

+ Запит плейлиста.
+ Завантаження кількості сегментів, зазначених у VOD Limit segments on startup.
+ Таймаут на тривалість сегмента.
+ Запит плейлиста.
+ Завантаження кількості сегментів, зазначених у VOD segments queue Step.
+ Таймаут на тривалість сегмента.
+ Запит плейлиста.
+ Завантаження кількості сегментів, зазначених у VOD segments queue Step.
+ ...

З'єднання не скидається, оскільки плейлист постійно оновлюється: надходять запити нового плейлиста з новими сегментами (аналогічно Live-потокам).

## Режим «smart» для оновлення HLS плейлиста

Smart-режим автоматично визначає оптимальний час для оновлення плейлиста. Його мета - отримати новий сегмент якомога раніше, уникаючи при цьому зайвих запитів до сервера.
Зазвичай вам не потрібно нічого налаштовувати, оскільки параметри за замовчуванням уже є оптимальними. Однак, якщо ви хочете змінити налаштування, нижче наведено докладний опис.

**Як це працює**

Кожен сегмент має певну тривалість (наприклад, 5 секунд). Smart-режим працює так:

+ Чекає майже до кінця поточного сегмента, а потім робить «ранній» запит.
+ Якщо новий сегмент ще не з'явився, він робить коротку «перевірку» через певний час (не частіше, ніж задано).
+ Додає невеликий «джиттер» (випадкове зміщення часу), щоб уникнути одночасних запитів до сервера.

Такий підхід забезпечує мінімальну кількість запитів і швидку доставку нових сегментів.

**Можливі сценарії**

+ Новий сегмент з'явився: наступний запит виконується «трохи раніше за кінець» поточного сегмента.
+ Нового сегмента ще немає: програма чекає короткий фіксований інтервал і повторює запит.
+ Плейлист порожній або без тривалості: режим тимчасово переходить на запасний алгоритм і скидає свій внутрішній стан. Це дозволяє коректно відновити роботу, коли плейлист знову стане доступним.

**Параметри**

Поточні значення за замовчуванням (tvlink/libs/streamlink/stream/hls/hls.py):
...
+ reload_early_offset_s (секунди)
    + Визначає, наскільки раніше «кінця сегмента» потрібно зробити запит.
    + 0.10 означає, що плейлист запитується приблизно за 100 мс до кінця сегмента.
    + Можна вказати від’ємне значення, щоб запитувати «трохи після кінця» (наприклад, -0.05). Це зменшить кількість «подвійних» запитів, але може спричинити невелику затримку.

+ reload_publish_slack_s (секунди)
    + Це запас часу для «перевірки», коли сегмент ще недоступний.
    + Працює разом із мінімальним інтервалом.

+ reload_idle_min (секунди)
    + Мінімальний інтервал між «перевірками», щоб уникнути надмірного навантаження на сервер.
    + Збільшення цього значення призведе до меншої кількості запитів, але може додати невелику затримку.

+ reload_jitter_ms (мілісекунди)
    + Випадкове зміщення часу очікування, яке запобігає синхронним запитам до сервера.
    + Типове значення: 20–50 мс.

**Внутрішні гарантії:**

+ Мінімальний технічний інтервал очікування — 0.25 секунди.
+ Занадто велике позитивне зміщення автоматично обмежується, щоб не перетворити очікування на майже нульове.

**Коли і що змінювати:**

+ Якщо ви хочете зменшити кількість «подвійних» запитів:
    + Трохи збільште reload_early_offset_s (наприклад, до 0.15–0.20) або зробіть його від’ємним (наприклад, -0.05…-0.10).
+ Якщо сервер скаржиться на часті запити:
    + Збільште reload_idle_min (наприклад, до 1.0–1.5).
+ Якщо вам потрібен максимально «свіжий» потік (мінімум затримки):
    + Залиште reload_early_offset_s у межах 0.05–0.10 і не збільшуйте reload_idle_min.

**Приклад логів і як їх читати:**

+ Next reload [smart] in 4.746s: base=5.000s changed=True streak=0
    + base — тривалість останнього сегмента (сек).
    + changed — плейлист оновився (з'явився новий сегмент).
    + streak — скільки разів поспіль плейлист НЕ оновлювався (0 — щойно оновився).
    + in 4.746s — через скільки секунд планується наступний запит.
+ Reloading playlist [smart]: base=5.000s changed=True streak=0 planned=4.871s | waited=4.746s elapsed=0.125s
    + planned — скільки секунд було заплановано для поточного циклу очікування.
    + waited — скільки часу фактично чекали.
    + elapsed — накладні витрати між циклами (час обробки тощо).

Зазвичай ви побачите 1–2 запити на кожен 5-секундний сегмент — це нормальна та бажана поведінка.

**Коротко про відмовостійкість**

+ Якщо плейлист тимчасово пошкоджений (немає сегментів або тривалості), smart-режим перейде на запасний алгоритм і скине свій внутрішній стан.
Це гарантує, що після відновлення він почне працювати «з чистого аркуша».
+ Перший цикл після запуску потоку вважається оновленим (changed=True), щоб уникнути зайвої початкової паузи.

{{% hint warning %}}
Якщо ви не впевнені, які саме параметри змінювати, **залиште налаштування за замовчуванням**.
Вони забезпечують оптимальний баланс між швидкістю та кількістю запитів.
{{% /hint %}}
